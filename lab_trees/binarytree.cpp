...
bool BinaryTree<T>::isOrdered(const Node* subRoot) const 
{
    ...
    if(tempL==NULL) 
        left = true;
    else if((tempL->elem) <= (subRoot->elem))
        left = isOrdered(tempL);

    if(tempR == NULL) 
        right = 1; 
    else if((tempR ->elem) >= (subRoot->elem))
        right = isOrdered(tempR);
...
template <typename T>
void BinaryTree<T>::printPaths(Node * subRoot, vector<int> vec) const
{
    bool flag = false;
...
int BinaryTree<T>::sumDistances(Node * subRoot, int i) const
{
    ...
    int left = sumDistances(subRoot->left, i ++);
    int right = sumDistances(subRoot->right , i++);
